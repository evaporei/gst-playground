...
static gboolean refresh_ui (CustomData *data) {
  ...
  if (gst_element_query_position (data->playbin, GST_FORMAT_TIME, &current)) {
    // block "value-changed" signal so slider_cb is not called
...
static void tags_cb (GstElement *playbin, gint stream, CustomData *data) {
  // this could possibly running in a GStreamer working thread, so we notify
...
static void error_cb (GstBus *bus, GstMessage *msg, CustomData *data) {
  ...
  g_printerr ("Error received from an element %s: %s\n", GST_OBJECT_NAME (msg->src), err->message);
...
static void analyze_streams (CustomData *data) {
  ...
  for (i = 0; i < n_audio; i++) {
    ...
    if (tags) {
      ...
      if (gst_tag_list_get_uint (tags, GST_TAG_LANGUAGE_CODE, &rate)) {
...
// called when application msg gets into bus
...
int main(int argc, char *argv[]) {
  ...
  g_object_set (data.playbin, "videosink", videosink, NULL);
